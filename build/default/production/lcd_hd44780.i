...
# 53 "C:/Program Files/Microchip/MPLABX/v6.05/packs/Microchip/PIC16Fxxx_DFP/1.3.42/xc8\pic\include\proc\pic16f627.h"
...
# 93
struct {
uint8_t LCDEntryModeByte;
uint8_t LCDFunctionSetByte;
uint8_t LCDDisplayControlByte;
} LCD;

# 102
inline void LCDSelectRegister(uint8_t);

# 107
void LCDWriteData(uint8_t);

# 120
inline void LCDClearDisplay();

# 125
inline void LCDReturnHome();

# 132
void LCDEntryModeSet(uint8_t);

# 137
void LCDFunctionSet(uint8_t);

# 142
void LCDDisplayControl(uint8_t);

# 149
void LCDSetCGRAMAddress(uint8_t);

# 154
void LCDSetDDRAMAddress(uint8_t);
...
# 178
inline void LCDCursorOn(void);

# 183
inline void LCDCursorOff();

# 188
inline void LCDCursorBlinkOn();

# 193
inline void LCDCursorBlinkOff();

# 198
inline void LCDDisplayOn();

# 203
inline void LCDDisplayOff();
...
# 28 "lcd_hd44780.c"
inline void LCDSelectRegister(uint8_t reg) {
PORTAbits.RA4 = (reg == LCD_RS_WRITE_DR || reg == LCD_RS_WRITE_IR);
PORTAbits.RA5 = (reg == LCD_RS_READ_BF_AC || reg == LCD_RS_READ_DR);
}
...
uint8_t bfach = 0;
LCDSelectRegister(LCD_RS_READ_BF_AC);
...
} while ((bfach & 0x08) == 0x08);

}

# 147
inline void LCDClearDisplay() {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCDWriteData(0b00000001);
}

# 155
inline void LCDReturnHome() {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCDWriteData(0b00000010);
}

# 165
void LCDEntryModeSet(uint8_t param) {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCD.LCDEntryModeByte = 0b00000100 | param;
LCDWriteData(LCD.LCDEntryModeByte);
}

# 176
void LCDFunctionSet(uint8_t param) {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCD.LCDFunctionSetByte = 0b00100000 | param;
LCDWriteData(LCD.LCDFunctionSetByte);
}

# 187
void LCDDisplayControl(uint8_t param) {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCD.LCDDisplayControlByte = 0b00001000 | param;
LCDWriteData(LCD.LCDDisplayControlByte);
}

# 198
void LCDSetCGRAMAddress(uint8_t addr) {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCDWriteData(0b01000000 | (addr & 0x3F));
}

# 208
void LCDSetDDRAMAddress(uint8_t addr) {
LCDSelectRegister(LCD_RS_WRITE_IR);
LCDWriteData(0b10000000 | (addr & 0x7F));
}
...
# 249
inline void LCDCursorOn() {
LCDDisplayControl(LCD.LCDDisplayControlByte | 0b00000010);
}
...
# 267
inline void LCDCursorBlinkOn() {
LCDDisplayControl(LCD.LCDDisplayControlByte | 0b00000001);
}
...
# 285
inline void LCDDisplayOn() {
LCDDisplayControl(LCD.LCDDisplayControlByte | 0b00000100);
}
...
while (*str != '\0') {
LCDSelectRegister(LCD_RS_WRITE_DR);
LCDWriteData(*str);
str++;
}
}
...
# 318
void LCDStoreCustom5x8Character(uint8_t code, const char* dataarr) {
char cnt;

LCDSetCGRAMAddress((code << 3) & 0b00111000);

for (cnt = 0; cnt != 8; cnt++) {
LCDSelectRegister(LCD_RS_WRITE_DR);
LCDWriteData(*dataarr);
dataarr++;
}
}
...
# 339
LCDFunctionSet(0b00000000);

# 345
LCDFunctionSet(0b00001000);
